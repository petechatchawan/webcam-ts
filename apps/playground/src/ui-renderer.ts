...
export class UiRenderer {
	...
	private populateResolutionOptions(): void {
		...
		for (const preset of getResolutionPresets()) {
			groups
				.get(preset.orientation)
				?.append(new Option(`${preset.label} · ${preset.width} × ${preset.height}`, preset.id));
		}
	...
	private renderError(snapshot: PlaygroundSnapshot): void {
		...
		this.errorMessage.textContent = `${snapshot.error.message}${
			snapshot.error.operation ? ` · ${snapshot.error.operation}` : ""
		}`;
	}

	private renderEvents(snapshot: PlaygroundSnapshot): void {
		const items = [...snapshot.events].reverse().map((entry) => {
